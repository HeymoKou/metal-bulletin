...
- 폐기된 소스: mining.com/moneytoday RSS, GDELT 2.0 API, 한국비철금속협회, KORES — 코드 삭제됨 (2026-05-18, git history에 보존). 헤드라인 오프토픽 다수 → snmnews만.
- KOMIS deferred (뉴스용) — KOMIS 비철 LME CASH는 우리 sett_cash와 100% 중복 (2026-05-08 Ni 18890.0 일치 검증). data.go.kr 자체 Open API 없음. (가격 cross-validation 용도로는 활용 중 — 위 KOMIS 섹션 참조)
...
## 테스트
- `uv run pytest tests/` — 125 (Python, 가격/뉴스)
- `npm run smoke` — 8 (JS Parquet 로드 + 스키마)
- `prek run --all-files` — lint gate (ruff F/I/B + 파일 위생). clone 후 `prek install` 1회 필요.
