...
defer (사용자 결정 필요):
- ⚠️ **HIGH**: 모든 단계 silent fail. 소스 1개 다 죽어도 workflow green. **freshness check / 최소 row threshold / 알림**: 사용자 결정 — 어떤 임계치? 알림 채널?
- ⚠️ **MED**: dedupe before classify → 첫 dup이 generic 헤드라인이면 metal 가진 dup 버려짐. 순서 바꾸거나 classify-aware dedupe. 영향 측정 후 결정.
- ⚠️ **MED**: Gemini JSON mode 미강제 (response_mime_type="application/json"). prompt injection 시 batch 전체 null 처리 → google-genai SDK structured output로 강화 가능.

→ deferred 3개는 별도 phase에서 다룰 가치 있음. 현재 데이터 정확성 영향 적음 (silent fail = 데이터 stale 정도, dedupe order = 일부 false negative, JSON mode = 환각 시 자체 fallback 작동).
